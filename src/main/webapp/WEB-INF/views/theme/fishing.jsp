...
<body>
	...
	<div class="page-wrapper">
		...
		<section class="page-header" style="background-image: url(<c:url value="/resources/images/banner_main/${BannerRespectivelyView8.getS_file_name()}"/>);">
			<div class="container">
				<h2>낚시</h2>
				...
				<div class="row">
					<div class="col-lg-8">
						<div class="tour-sorter-one"></div>
							<c:forEach var="plist7" items="${plist7}">
								<div class="tour-two__single tour-one__single">
									<div class="tour-two__image-wrap">
										<div class="tour-one__image" style="width:350px;">
											<img src='<c:url value="/resources/images/product_package/${plist7.getS_file_name()}"/>' height=400 alt=""> 
									...
									<div class="tour-one__content">
										<div class="tour-two__top">
											<div class="tour-two__top-left">
												<h3><a href="detailInfo?PID=${plist7.getPid()}">${plist7.getProductname()}</a></h3>
											</div>
											<div class="tour-two__right">
												<p><span>$<fmt:formatNumber value="${plist7.getAdultprice()}" pattern="###,###,###"/></span> <br> 인당 가격</p>
											</div>
										</div>
										<div class="tour-two__text">
											<c:out escapeXml="false" value="${fn:replace(fn:replace(plist7.getOverview(), '&lt;', '<'), '&gt;', '>')}"/>
										</div>
										<ul class="tour-one__meta list-unstyled">
											<li><a href="detailInfo?PID=${plist7.getPid()}"><i class="far fa-clock"></i>${plist7.getStartravelperiod()}~${plist7.getArrivaltravelperiod()}</a></li>
											<li><a href="detailInfo?PID=${plist7.getPid()}"><i class="far fa-user-circle"></i> ${plist7.getMinreservation()}+이상</a></li>
											<li><a href="detailInfo?PID=${plist7.getPid()}"><i class="far fa-map"></i>${plist7.getTheme()}</a></li>
										</ul>
									</div>
								</div>
							</c:forEach>
						<div class="post-pagination">
							...
							<c:choose>
								<c:when test="${Paging.pageNo eq Paging.firstPageNo }">
									<a class="disabledLink" href="fishing?page=${Paging.prevPageNo}"><i class="fa fa-angle-left"></i></a>
								</c:when>
								<c:otherwise>
									<a class="page-link" href="fishing?page=${Paging.prevPageNo}"><i class="fa fa-angle-left"></i></a>
								</c:otherwise>
							</c:choose>
							<!-- 페이지 갯수만큼 버튼 생성 -->
							<c:forEach var="i" begin="${Paging.startPageNo }" end="${Paging.endPageNo }" step="1">
								<c:choose>
									<c:when test="${i eq Paging.pageNo }">
										<a class="active" href="fishing?page=${i}"><c:out value="${i }"/></a>
									</c:when>
									<c:otherwise>
										<a href="fishing?page=${i}"><c:out value="${i }"/></a>
									</c:otherwise>
								</c:choose>
							</c:forEach>
							...
							<c:choose>
								<c:when test="${Paging.pageNo eq Paging.finalPageNo }">
									<a class="disabledLink" href="fishing?page=${Paging.nextPageNo}"><i class="fa fa-angle-right"></i></a>
								</c:when>
								<c:otherwise>
									<a href="fishing?page=${Paging.nextPageNo}"><i class="fa fa-angle-right"></i></a>
								</c:otherwise>
							</c:choose>
						</div>
					</div>
