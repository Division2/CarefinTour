...
<body>
	...
	<div class="page-wrapper">
		...
		<section class="page-header" style="background-image: url(<c:url value="/resources/images/backgrounds/honey.png"/>);">
			<div class="container">
				<h2>허니문</h2>
				...
				<div class="row">
					<div class="col-lg-8">
						<div class="tour-sorter-one"></div>
							<c:forEach var="plist6" items="${plist6}">
								<div class="tour-two__single tour-one__single">
									<div class="tour-two__image-wrap">
										<div class="tour-one__image">
											<img src='<c:url value="/resources/images/product_package/${plist6.getS_file_name()}"/>' height=292 width=270 alt=""> 
									...
									<div class="tour-one__content">
										<div class="tour-two__top">
											...
											<div class="tour-two__right">
												<p><span>${plist6.getAdultprice()}$</span> <br> 인당ㄴ 가격</p>
											</div>
										</div>
										...
										<ul class="tour-one__meta list-unstyled">
											...
											<li><a href="detailInfo?PID=${plist6.getPid()}"><i class="far fa-user-circle"></i> ${plist6.getMinreservation()}+이상</a></li>
											<li><a href="detailInfo?PID=${plist6.getPid()}"><i class="far fa-map"></i>${plist6.getTheme()}</a></li>
										</ul>
									</div>
								</div>
							</c:forEach>
						<div class="post-pagination">
							...
							<c:choose>
								<c:when test="${Paging.pageNo eq Paging.firstPageNo }">
									<a class="disabledLink" href="honeymoon?page=${Paging.prevPageNo}"><i class="fa fa-angle-left"></i></a>
								</c:when>
								<c:otherwise>
									<a class="page-link" href="honeymoon?page=${Paging.prevPageNo}"><i class="fa fa-angle-left"></i></a>
								</c:otherwise>
							</c:choose>
							<!-- 페이지 갯수만큼 버튼 생성 -->
							<c:forEach var="i" begin="${Paging.startPageNo }" end="${Paging.endPageNo }" step="1">
								<c:choose>
									<c:when test="${i eq Paging.pageNo }">
										<a class="active" href="honeymoon?page=${i}"><c:out value="${i }"/></a>
									</c:when>
									<c:otherwise>
										<a href="honeymoon?page=${i}"><c:out value="${i }"/></a>
									</c:otherwise>
								</c:choose>
							</c:forEach>
							...
							<c:choose>
								<c:when test="${Paging.pageNo eq Paging.finalPageNo }">
									<a class="disabledLink" href="honeymoon?page=${Paging.nextPageNo}"><i class="fa fa-angle-right"></i></a>
								</c:when>
								<c:otherwise>
									<a href="honeymoon?page=${Paging.nextPageNo}"><i class="fa fa-angle-right"></i></a>
								</c:otherwise>
							</c:choose>
						</div>
					</div>
